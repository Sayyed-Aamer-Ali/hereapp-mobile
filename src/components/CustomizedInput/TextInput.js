  ...
  (
    ...
    return (
      <View style={[styles.mainCont, style]}>
        ...
        <View
          style={[
            styles.container(fieldInfo?.error?.length > 0 ? true : false),
            InputContStyle,
          ]}>
          {/* Left Icon */}
          {fieldInfo?.leftIcon ||
          ...
          fieldInfo?.type == 'password' ? (
            <View style={styles.leftIconCont}>
              {fieldInfo?.leftIcon && <>{fieldInfo?.leftIcon}</>}

              {fieldInfo?.type === 'email' && <Icons.Email />}

              {fieldInfo?.type === 'password' && <Icons.Password />}
            </View>
          ) : null}
          ...
          <View style={{flex: 1}}>
            {fieldInfo?.type == 'nonEditable' ? (
              <TouchableOpacity
                ...
                <Text
                  style={styles.inputnonEditableText(
              ...
              <>
                {!isPhoneNumber ? (
                  <TextInput
                    ...
                    secureTextEntry={
                      fieldInfo?.type === 'password' && showPassword
                    }
                    keyboardType={
                      fieldInfo?.type === 'email'
                        ? 'email-address'
                    ...
                    returnKeyType={
                      props?.returnKeyType ? props?.returnKeyType : 'next'
                    }
        ...
        {fieldInfo?.error?.length > 0 ? (
          <Text style={styles.ErrorText}>{fieldInfo?.error}</Text>
        ) : null}
      </View>
    );
  },
