...
const Login = ({navigation, route}) => {
  ...
  } = useSelector(state => state.auth.rememberMeCreds);
    ...
    try {
      ...
      if (response) {
        if (
          ...
          response?.data?.user?.actingRole !== userType.toUpperCase()
        ) {
          let vowel = userType === 'Student' ? 'a' : 'an';
          AlertService.toastPrompt(
            `Please select the correct role, you are not ${vowel} ${userType.toLowerCase()}`,
            'error',
          );
          return;
        }
  ...
  return (
    <MainLayout loader={loader}>
      <Header title={'Sign In'} rightIcons={false} />
              ...
              <Text
                style={[
                  styles.regText,
                  {
                    ...
                    marginTop:
                      Platform.OS == 'android' ? UtilityMethods.hp(0.7) : 0,
                  },
                ]}>
                Forgot Password?
              </Text>
