...
import { View, Text, FlatList, Image, TouchableOpacity, Keyboard, Alert } from 'react-native';
import Icon from 'react-native-vector-icons/Ionicons';
import AntDesign from 'react-native-vector-icons/AntDesign';
import FontAwesome6 from 'react-native-vector-icons/FontAwesome6';

import { CustomFlatList, CustomizedInput, Header, MainLayout } from '../../../components';
...
let missedClasses = useSelector((state) => state.temp.AllMissedClassesName);
  ...
  const [filterData, setFilterData] = useState(missedClasses?.AllMissedClassesName??[]);
...
let index = 0;
  ...
  const renderItem = ({ item }) => (
    ...
    >
       <FontAwesome6 name="clock-rotate-left" size={20} color={Colors.LIGHT_GRAY} />
