...
const Attendance = ({navigation, route}) => {
  ...
  const handleSubmit = async () => {
    ...
    if (
      !location?.sucess &&
      attendanceData?.classDetail.geoTracking == 'enable'
    ) {
    ...
    try {
      ...
      if (!isNaN(attemptsLeft1)) {
        ...
        if (attemptsLeft1 >= 0) {
          setErrorMessage(
            `Wrong Code, You have ${attemptsLeft1} Attempts Left.`,
          );
        } else {
        ...
        if (e.includes('attendance has been conducted')) {
          setErrorMessage(
            'Class attendance has been conducted. Please request excused attendance',
          );
          setAttemptsLeft(0);
        } else {
    ...
    <MainLayout loader={loader}>
      ...
      <ScrollView
        ...
        <Text style={styles.title}>
          Enter the One Time Code to Join the Class
        </Text>
        ...
        <Button
          ...
          Icon={<Icons.Right />}
          onPress={
            attemptsLeft > 0 ? handleSubmit : handleRequestExcusedAbsence
          }
        />
        ...
        <View style={styles.container}>
          ...
          <View style={styles.noteBody}>
            ...
            <Text style={styles.noteText}>
              You can attempt for the maximum of{' '}
              <Text style={styles.highlightText}> {attemptsLeft} Times.</Text>
            </Text>
          </View>
          <View style={styles.noteBody}>
            <Text style={styles.bulletPoint}>•</Text>
          ...
          <View style={styles.noteBody}>
            ...
            <Text style={styles.noteText}>
              {`Your location ${
                item?.geoTracking == 'enable' ? 'will be' : 'will not be'
              } recorded at the time of marking the attendance.`}
            </Text>
          </View>
        </View>
      </ScrollView>
